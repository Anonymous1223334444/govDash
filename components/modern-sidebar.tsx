...
import { 
  ...
  Settings, ChevronLeft, ChevronRight, Chrome as Home, TrendingUp, 
  Menu, X, Sparkles, Zap, Activity, ArrowRight, Dot, Search, Layout
} from "lucide-react"
...
const menuItems = [
  ...
  },
  {
    ...
    bgColor: "bg-sky-50/80 dark:bg-sky-950/50",
    gradient: "from-sky-500/10 via-sky-400/5 to-sky-600/10",
...
export function ModernSidebar({ activeTab, onTabChange, collapsed, setCollapsed, user, blockedCount = 0 }: ModernSidebarProps) {

  const [mobileOpen, setMobileOpen] = useState(false)
  const [hoveredItem, setHoveredItem] = useState<string | null>(null)
  const roleLabel = getRoleConfig(user?.role).label

  const sidebarMenu = menuItems.map((item) =>
    item.id === "actions-bloquees"
      ? {
          ...item,
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative flex items-center justify-between p-6 border-b border-sidebar-border/60 min-h-[5rem] bg-gradient-to-r from-sidebar to-sidebar/90">
          ...
          {(!collapsed || mobileOpen) && (
            // <div className="flex items-center space-x-3 flex-1 min-w-0 animate-slide-in z-10">
              <div className="relative flex items-center h-8">
                <img
                  src="/penslogo.png"
                  alt="PENS"
                  className="block h-full w-auto max-w-[160px] object-contain select-none pointer-events-none"
                />
              </div>
            // </div>
          )}
          ...
          <div className="flex items-center space-x-2 z-10">
            <Button
              ...
              className="hidden lg:flex h-10 w-10 text-foreground/70 hover:text-foreground hover:bg-sidebar-accent/50 transition-all duration-300 hover:scale-110 rounded-xl"
            >
              {collapsed ? (
            ...
            <Button
              ...
              size="icon"
              onClick={() => setMobileOpen(false)}
              className="lg:hidden h-10 w-10 text-foreground/70 hover:text-foreground hover:bg-sidebar-accent/50 transition-all duration-300 hover:scale-110 rounded-xl"
            >
              <X className="h-5 w-5" />
            </Button>
          </div>
          ...
          <div className="flex items-center space-x-4 animate-slide-in stagger-1">
            <div className="relative group">
              <Avatar className="h-14 w-14 flex-shrink-0 ring-4 ring-sidebar-primary/20 transition-all duration-500 group-hover:ring-sidebar-primary/40 group-hover:scale-105">
                <AvatarImage src={user?.image || "/african-man-professional.png"} />
            ...
            {(!collapsed || mobileOpen) && (
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-foreground truncate text-lg">{user?.name || "Utilisateur"}</p>
        ...
        {/* Enhanced Navigation */}
                <nav className="flex-1 p-3 flex flex-col justify-between overflow-y-auto scrollbar-thin">
      ...
      <Button
        ...
        className={cn(
          "fixed bottom-6 right-6 z-50 lg:hidden h-12 w-12 shadow-xl backdrop-blur-lg",
          "bg-background/95 border-border/50 hover:bg-background hover:shadow-2xl",
